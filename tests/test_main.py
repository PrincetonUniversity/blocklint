...
def test_main(mocker):
    ...
    assert mock_print.call_args_list == [
        mocker.call("stdin:" + str(i) + ':1: use of "aba"') for i in range(11, 21)
    ]
...
def test_get_args_wordlists(mocker):
    ...
    args = bl.get_args(
        (
            "--end-pos --blocklist test,test "
            "--exactlist test2,test2 "
            "--wordlist test3,test3"
        ).split()
    )
    assert args == {
        "blocklist": ["test"],
    ...
    args = bl.get_args(("-e --blocklist test --exactlist test --wordlist test").split())
    assert args == {
        "blocklist": ["test"],
        "exactlist": [],
        "files": [],
    ...
    args = bl.get_args(("--blocklist test1 --exactlist test --wordlist test").split())
    assert args == {
        "blocklist": ["test1"],
        "exactlist": [],
        "files": [],
    ...
    assert args == {
        ...
        "stdin": False,
        "skip_files": set(
            ("tests/sample_files/test.py", "tests/sample_files/test.txt")
        ),
        "max_issue_threshold": None,
        "wordlist": [],
    }
...
def test_generate_re(mocker):
    ...
    mocker.patch(
        ...
        + ("i" if len(x) > 1 and x[1] == re.IGNORECASE else ""),
    )
    assert bl.generate_re({"blocklist": [], "wordlist": [], "exactlist": []}) == {}

    assert bl.generate_re(
        {"blocklist": ["bab"], "wordlist": ["cac"], "exactlist": ["dad"]}
    ) == {
...
def test_generate_re_matches():
    regexes = bl.generate_re(
        {
            "blocklist": ["bab", "longerwordtotest"],
            "wordlist": ["cac"],
            "exactlist": ["dad"],
        }
    )
    ...
    assert list(
        bl.check_line("this is a l!o@n#g$e%r^w&o*r(d)t-o_t+e=s/t", regexes, "test", 1)
    ) == [
        'test:1:11: use of "longerwordtotest"'  # special
    ]
    assert list(
        bl.check_line(
            "more l\\o|n?g[e]r{w}o,r.d<t>o`t~e;s:t", regexes, "test", 2, end_pos=True
        )
    ) == [
    ...
    assert list(
        bl.check_line(
            "aCAC not found, but !c@A?c. is ", regexes, "test", 4, end_pos=True
        )
    ) == [
    ...
    assert list(
        bl.check_line('adad d@ad and DaD are missed, but not ,dad"', regexes, "test", 5)
    ) == [
    ...
    assert list(
        bl.check_line(
            "int test(std::vector<int> blacklist, int master){", regexes, "test", 1
        )
    ) == ['test:1:27: use of "blacklist"', 'test:1:42: use of "master"']
