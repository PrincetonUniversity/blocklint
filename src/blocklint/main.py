...
def main(args=None):
    ...
    if args["stdin"]:
        total_issues += process_file(sys.stdin, "stdin", word_checkers, args["end_pos"])
    else:
        for file in args["files"]:
            with open(file, "r") as handle:
                total_issues += process_file(
                    handle, file, word_checkers, args["end_pos"]
                )
    if (
        args["max_issue_threshold"] is not None
...
def process_file(input_file, file_name, word_checkers, end_pos):
    ...
    try:
        ...
        for i, line in enumerate(lines, 1):
            for match in check_line(line, word_checkers, file_name, i, end_pos):
                num_matched += 1
                print(match)
...
def get_args(args=None):
    ...
    parser.add_argument(
        "--stdin", action="store_true", help="Use stdin as input instead of file list"
    )
    parser.add_argument(
        "--max-issue-threshold",
        type=int,
    ...
    present_config_files = [path for path in config_paths if os.path.exists(path)]
    config = configparser.ConfigParser()
    for path in present_config_files:
        config.read(path)
    config_settings = {}  # type: dict[str, str | bool | int | set[str]]
    ...
    if args["skip_files"] is not None:
        ...
        skip_files = [
            os.path.abspath(path) for line in skip_files for path in line.split(",")
        ]
    ...
    if (
        args["blocklist"] is None
        and args["wordlist"] is None
        and args["exactlist"] is None
    ):
    ...
    if args["skip_files"] is not None:
        files = [
            file for file in files if os.path.abspath(file) not in args["skip_files"]
        ]
...
def generate_re(args):
    ...
    for word in args["wordlist"]:
        result[word] = re.compile(word_boundaries(ignore_special(word)), re.IGNORECASE)
